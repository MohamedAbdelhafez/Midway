...
u0 = []
signal = 0.01*np.tanh(np.linspace(0,total_time, steps)/10)
u0.append(signal)
...
for ii in range (clusters+1):
    ...
    if (len(string) <5):
        hosts.append(server+string+":8000")
    else:
        s = int(string[0:4])
        e = int(string[6:10])
        for jj in range (e-s+1):
            hosts.append(server+str(s+jj).zfill(4)+":8000")

idx = hosts.index(node_name+":8000") 
if (idx==0):
    job_name = "ps"
    task_index = 0
else:
