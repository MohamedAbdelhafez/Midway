...
class TensorflowState:
    ...
    def midway(self):
        ...
        self.node_name = str(os.environ["SLURMD_NODENAME"])
        ...
        for ii in range (clusters+1):
            ...
            if (len(string) <5):
                self.hosts.append(server+string+":2222")
            else:
                s = int(string[0:4])
                e = int(string[6:10])
                for jj in range (e-s+1):
                    self.hosts.append(server+str(s+jj).zfill(4)+":2222")
        print(self.hosts)
        print(self.node_name)
        idx = self.hosts.index(self.node_name+":2222") 
        if (idx==0):
            self.job_name = "ps"
            self.task_index = 0
        else:
            self.job_name = "worker"
            self.task_index = idx -1
        self.cluster = tf.train.ClusterSpec( {"ps" : [self.hosts[0]], "worker": self.hosts[1:] } )
        self.server = tf.train.Server(server_or_cluster_def=self.cluster,
                                 job_name=self.job_name, task_index=self.task_index)
